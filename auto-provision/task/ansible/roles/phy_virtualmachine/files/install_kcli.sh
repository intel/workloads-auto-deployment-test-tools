...
if [ "$ALIAS" != "0" ]; then
  ...
  echo -e '#/bin/bash\n'$engine run -p 9000:9000 --net host -it --rm --security-opt label=disable -v $HOME/.kcli:/root/.kcli $SSHVOLUME $VOLUMES '-v $PWD:/workdir --entrypoint=/usr/bin/klist.py quay.io/karmab/kcli' > $HOME/klist.py
case $shell in
bash|zsh)
