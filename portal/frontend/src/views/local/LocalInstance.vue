...
<div>
  ...
  <div class="card mt-2" v-show="!showtestcases">
    <div class="card-header text-center">Servers</div>
    <div>
      <DataTable
        :value='instances'
      ...
      >
        ...
        <template #body="slotProps" v-if="col.field === 'status'">
            <Button
             class="p-button-raised p-button-rounded p-button-success p-button-sm button"
             :label="slotProps.data.status"
             v-if="slotProps.data.status === 'FREE'">
            </Button>
            <Button
             class="p-button-raised p-button-rounded p-button-info p-button-sm button"
             :label="slotProps.data.status"
             v-else-if="slotProps.data.status === 'DELETED'">
            </Button>
            <Button
             class="p-button-warning p-button-sm p-button-raised p-button-rounded button"
             :label="slotProps.data.status"
             v-else-if="slotProps.data.status === 'DELETING'">
            </Button>
            <Button
             class="p-button-danger p-button-sm p-button-raised p-button-rounded button"
             :label="slotProps.data.status"
             v-else-if="slotProps.data.status === 'IN_USE'">
            </Button>
            <Button
             class="p-button-danger p-button-sm p-button-raised p-button-rounded button"
             :label="slotProps.data.status"
             v-else-if="slotProps.data.status === 'CREATE_FAILED'">
            </Button>
            <Button
          ...
          <template #body="slotProps" v-else-if="col.field === 'username'">
            <Button
             ...
             @click="copy"
             :data-clipboard-text="slotProps.data.username"
             v-if="slotProps.data.username !== null">
            </Button>
          </template>
          <template #body="slotProps" v-else-if="col.field === 'password'">
            <Button
             ...
             class="p-button-warning p-button-rounded"
             @click="copy"
             :data-clipboard-text="slotProps.data.password"
  ...
  <Dialog
    :header="'Actions for instance'"
    :visible.sync="displayAction"
    :style="{width: '20%'}"
    :maximizable="false"
    :modal="true"
    v-if="displayAction"
  ...
  <Dialog
    :header="'Duration extention Days'"
    :visible.sync="displayExtention"
    :style="{width: '20%'}"
    :maximizable="false"
    :modal="true"
    v-if="displayExtention"
  >
    <InputText
